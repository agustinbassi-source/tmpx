    ...
    public class ReportService
    {
                                            ...
                                            if (classx2.Data == classx.Data)
                                            {
                                                foreach (var functionx2 in classx2.Items)
                                                {
                                                    if (functionx2.Data == functionx.Data && !level2.Items.Select(x => x.DataRaw).Contains(filex2.Data))
                                                    {
                                                        ...
                                                        if (filex2.Path.Trim().IndexOf(".vb") > 0 
                                                            &&
                                                            (functionx2.LineCode.IndexOf("."+ functionx2.Data) > -1 ||
                                                            functionx2.LineCode.IndexOf("New " + functionx2.Data) > -1))
                                                        {
                                                            addRow = true;
                                                        }
                                                        else if (functionx2.LineCode.IndexOf("." + functionx2.Data) > -1 ||
                                                            functionx2.LineCode.IndexOf("new " + functionx2.Data) > -1)
                                                        {
                                                            addRow = true;
                                                        }
                                                        ...
                                                        if (addRow)
                                                        {
                                                            ...
                                                            level2.Items.Add(new ReportItem
                                                            {
                                                                ...
                                                                DataRaw = filex2.Data.Trim(), // "cls_tito.vb"
                                                                Code = functionx2.LineCode.Trim(), // if (funtion....
                                                                Path = filex2.Path.Trim(), // x:source/cls.vb
                                                                LineNumber = functionx2.LineNumber
        ...
        public Report BuildReport(Project proyect)
        {
            ...
            foreach (var file in proyect.ProyectFiles)
            {
                ...
                foreach (var item in filesNameSpace)
                {
                    ...
                    string path = item.File.FullName;
      ...
      "\\Wigos System\\WSI.Common.Test"
        ...
        private ProjectFile GetFile(string path)
        {
            ...
            foreach (var lineSource in code)
            {
                if (lineSource.IndexOf("ProfilePermissions") > -1)
                { }

                if (lineSource.IndexOf("static") > -1 && lineSource.IndexOf("(") > -1 && lineSource.IndexOf("public") > -1)
                { }




                var line = lineSource.Replace(" static ", " ");
